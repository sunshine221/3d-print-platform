...
const STATUS_MAP: Record<string, { color: string; label: string }> = {
  ...
  rejected: { color: 'bg-white/5 text-void-400 border border-white/5', label: '已拒绝' },
  closed: { color: 'bg-white/5 text-void-400 border border-white/5', label: '已关闭' },
};
...
export default function MyInquiriesPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-void-100 mb-4">我的询价</h2>
      {inquiries.length === 0 ? (
        <p className="text-void-400 py-8 text-center">暂无询价</p>
      ) : (
        <div className="space-y-3">
          {inquiries.map((inq) => {
            const st = STATUS_MAP[inq.status] || { color: 'bg-white/5 text-void-400 border border-white/5', label: inq.status };
            return (
              <Link
                key={inq.id}
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-void-300">{inq.inquiryNo}</p>
                    <p className="text-sm mt-1 text-void-200">
                      {inq.productName || '未指定产品'}
                      {inq.desiredMaterial && <span className="text-void-400 ml-2">· {inq.desiredMaterial}</span>}
                      {inq.desiredColor && <span className="text-void-400 ml-1">· {inq.desiredColor}</span>}
                    </p>
